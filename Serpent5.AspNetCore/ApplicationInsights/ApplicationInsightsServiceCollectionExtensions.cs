...
{
    /// <summary>
    /// Adds a <see cref="ITelemetryInitializer" /> that sets the Cloud Role Name in Application Insights.
    /// </summary>
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(cloudRoleName);

        return serviceCollection.AddSingleton<ITelemetryInitializer>(_ => new CloudRoleNameTelemetryInitializer(cloudRoleName));
    }

    /// <summary>
    /// Adds a <see cref="ITelemetryInitializer" /> that sets the Authenticated User ID in Application Insights to the
